...
import { Persons, Star, Copy } from "@gravity-ui/icons";
...
export default function AdminSummaryCards({ users, prompts }) {
  ...
  const totalReviews =
    prompts?.reduce((sum, p) => sum + (p.rating ? 1 : 0), 0) ?? 0;
  const totalCopies =
    prompts?.reduce((sum, p) => sum + (p.copyCount ?? 0), 0) ?? 0;
      ...
      {CARDS.map(({ icon: Icon, label, badgeColor }, i) => (
        ...
        >
          <div className="flex items-start justify-between">
            ...
            <span
              className={`${jetbrainsMono.className} text-[9px] font-bold px-2 py-1 rounded-md tracking-[0.06em] ${badgeColor}`}
            >
              {values[i].badge}
            </span>
          </div>
          ...
          <div>
            <p
              className={`${jetbrainsMono.className} text-[10px] text-white/25 tracking-widest mb-2`}
            >
              {label}
            </p>
            <p
              className={`${jetbrainsMono.className} text-[26px] md:text-[28px] font-bold text-white leading-none`}
            >
              {values[i].value}
            </p>
          </div>

          <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-linear-to-r from-[#AAFF00]/40 via-[#AAFF00]/20 to-transparent" />
        </div>
      ))}
    </div>
  );
